...
<div class="mx-5 mt-5 flex flex-col gap-y-3">
    <x-breadcrumbs
        :links="[
            ...
            ['label' => 'Tambah', 'url' => url('/users/create')],
        ]" />

    <div class="flex flex-col gap-y-5">
        <x-header title="Tambah Data Pengguna" subtitle="Masukkan data pengguna di sini" />

        <section class="bg-white rounded-2xl shadow-md p-5">
            <form class="flex flex-col gap-y-4" action="/users" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menyimpan data pengguna ini?')">
                @csrf
                @method('POST')
                <x-input label="Nama" name="name" placeholder="Masukkan nama pengguna" required autofocus />
