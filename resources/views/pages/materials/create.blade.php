...
<div class="mx-5 mt-5 flex flex-col gap-y-3">
    <x-breadcrumbs
        :links="[
            ...
            ['label' => 'Tambah', 'url' => url('/materials/create')],
        ]" />

    <div class="flex flex-col gap-y-5">
        <x-header title="Tambah Data Bahan" subtitle="Masukkan data bahan di sini" />

        <section class="bg-white rounded-2xl shadow-md p-5">
            <form class="flex flex-col gap-y-4" action="/materials" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menyimpan data bahan ini?')">
                @csrf
                @method('POST')
                <x-input label="Nama" name="name" placeholder="Masukkan nama bahan" required autofocus />
                <x-select label="Satuan" name="unit" placeholder="Masukkan unit bahan" required autofocus :options="$units" valueKey="code" labelKey="name" />
