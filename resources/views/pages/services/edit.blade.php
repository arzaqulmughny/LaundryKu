...
<div class="mx-5 mt-5 flex flex-col gap-y-3">
    <x-breadcrumbs
        ...
        ['label' => 'Edit', 'url' => url('/services/' . $service->id . '/edit')],
    ]"
    ...
    <section class="bg-white rounded-2xl shadow-md p-5">
        <form class="flex flex-col gap-y-4" action="/services/{{ $service->id }}" method="POST">
            @method('PUT')
            @csrf
            <x-input label="Nama" name="name" placeholder="Masukkan nama layanan" required autofocus value="{{ $service->name }}" />
            <x-input label="Deskripsi" name="description" placeholder="Masukkan deskripsi layanan" required value="{{ $service->description }}" />
            <x-input label="Harga" name="price" placeholder="Masukkan harga layanan" required value="{{ $service->price }}" />
            <x-select label="Satuan" name="unit" required value="{{ $service->unit }}" :options="['kg', 'pcs']" />
            <x-checkbox label="Aktif" name="active" checked="{{ $service->active == 1 }}" />
...
<form action="/services/{{ $service->id }}" method="POST" style="display: none;" id="delete-form">
    @method('DELETE')
    @csrf
</form>
@endsection
