...
<div class="mx-5 mt-5 flex flex-col gap-y-3">
    <x-breadcrumbs
        ...
        ['label' => 'Satuan', 'url' => url('/units')],
        ['label' => 'Edit', 'url' => url('/units/' . $unit->id . '/edit')],
    ]"
        current="Edit Data Satuan" />
    <x-header title="Edit Data Satuan" subtitle="Edit data satuan di sini" />

    <section class="bg-white rounded-2xl shadow-md p-5">
        <form class="flex flex-col gap-y-4" action="/units/{{ $unit->id }}" method="POST">
            @method('PUT')
            @csrf
            <x-input label="Kode" :value="$unit->code" name="code" placeholder="Masukkan kode satuan" required autofocus />
            <x-input label="Nama" :value="$unit->name" name="name" placeholder="Masukkan nama satuan" required autofocus />
...
<form action="/units/{{ $unit->id }}" method="POST" style="display: none;" id="delete-form">
    @method('DELETE')
    @csrf
</form>
@endsection
...
<script>
     ...
     */
    const onDelete = () => {
        if (!confirm('Apakah Anda yakin ingin menghapus data Satuan ini?')) {
            return;
        }
