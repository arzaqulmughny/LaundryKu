...
<div class="mx-5 mt-5 flex flex-col gap-y-3">
    <x-breadcrumbs
        :links="[
            ...
            ['label' => 'Tambah', 'url' => url('/units/create')],
        ]" />

    <div class="flex flex-col gap-y-5">
        <x-header title="Tambah Data Satuan" subtitle="Masukkan data satuan di sini" />

        <section class="bg-white rounded-2xl shadow-md p-5">
            <form class="flex flex-col gap-y-4" action="/units" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menyimpan data Satuan ini?')">
                @csrf
                @method('POST')
                <x-input label="Kode" name="code" placeholder="Masukkan kode satuan" required autofocus />
                <x-input label="Nama" name="name" placeholder="Masukkan nama satuan" required autofocus />
