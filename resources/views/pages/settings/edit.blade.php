...
<div class="mx-5 mt-5 flex flex-col gap-y-3">
    <x-breadcrumbs
        ...
        ['label' => $setting->label, 'url' => url('/settings/' . $setting->id . '/edit')],
    ]"
    ...
    <section class="bg-white rounded-2xl shadow-md p-5">
        <form class="flex flex-col gap-y-4" action="/settings/{{ $setting->id }}" method="POST" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <x-input label="Nama Pengaturan" name="label" placeholder="" required value="{{ $setting->label }}" disabled />
            <x-input :type="$setting->type" label="Nilai" name="value" placeholder="" required autofocus value="{{ $setting->value ?? $setting->default_value }}"/>
...
<form action="/settings/{{ $setting->id }}/reset" method="POST" style="display: none;" id="delete-form">
    @method('DELETE')
    @csrf
</form>
@endsection
...
<script>
     ...
     */
    const onDelete = () => {
        if (!confirm('Apakah Anda yakin ingin mereset pengaturan ini?')) {
            return;
        }
