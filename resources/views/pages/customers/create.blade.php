...
<div class="mx-5 mt-5 flex flex-col gap-y-3">
    <x-breadcrumbs
        :links="[
            ...
            ['label' => 'Tambah', 'url' => url('/customers/create')],
        ]" />

    <div class="flex flex-col gap-y-5">
        <x-header title="Tambah Data Pelanggan" subtitle="Masukkan data pelanggan di sini" />

        <section class="bg-white rounded-2xl shadow-md p-5">
            <form class="flex flex-col gap-y-4" action="/customers" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menyimpan data pelanggan ini?')">
                @csrf
                @method('POST')
                <x-input label="Nama" name="name" placeholder="Masukkan nama pelanggan" required autofocus />
                <x-input label="No. Hp" name="phone" placeholder="Masukkan No. Hp" required />
                <x-input label="Alamat" name="address" placeholder="Masukkan alamat pelanggan" required />
                <x-input label="Email" name="email" placeholder="Masukkan email pelanggan" />
